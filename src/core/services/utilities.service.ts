...
export class UtilitiesService {
    ...
    parseDate(dateStr: string, showTime: boolean = false) {
        let retVal = null;
        if (dateStr != '') {
            retVal = new Date(dateStr);
            let dateText = dateStr.split('-');
    ...
    stringifyDate(date: Date, showTime: boolean = false) {
        if (date) {
            let dateText = [];
    ...
    stringifyDateWithOffset(date: Date, showTime: boolean = false) {
        if (showTime) {
    ...
    isTablet() {
        // TODO:
        const isTablet = false;//this.deviceService.isTablet();
        return isTablet;
    }
}
